...
export function FavoritePeople() {
  ...
  const topDirectors = data?.results.slice(0, 5) ?? []
  ...
  if (error || topDirectors.length === 0) {
    return null
  }
    ...
    <div className="rounded-lg border bg-card p-4 sm:p-6">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground sm:text-sm">
            Seus diretores mais assistidos
          </p>
        </div>
      ...
      <div className="space-y-3">
        {topDirectors.map((director, index) => (
          <div
            key={director.id}
            onClick={() => router.push(`/dashboard/people/${director.tmdbId}`)}
          ...
          >
            ...
            {director.profilePath ? (
              <Image
                src={
                  tmdbService.getProfileUrl(director.profilePath, "w185") || ""
                }
                alt={director.name}
                width={48}
                height={48}
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-3 text-xs text-muted-foreground">
                <span>{director.appearances} filmes</span>
                <div className="flex items-center gap-1">
                  <Star className="size-3 fill-yellow-400 text-yellow-400" />
                  <span>{director.avgRating.toFixed(1)}</span>
                </div>
              </div>
            </div>
      ...
      {data && data.total > 5 && (
        ...
        >
          Ver todos os {data.total} diretores →
        </button>
      )}
    </div>
  )
